...
add_action('admin_init', function () {
	...
	add_settings_field(
		'threew_about_contact_email',
		__('About Contact Email', 'threew-2025'),
		function () {
			$value = esc_attr(get_option('threew_about_contact_email', ''));
			echo '<input type="email" id="threew_about_contact_email" name="threew_about_contact_email" value="' . $value . '" class="regular-text" />';
			echo '<p class="description">' . esc_html__('Override the recipient for the About page contact form. Leave blank to use the site admin email.', 'threew-2025') . '</p>';
		},
		'general',
		'default'
	);
 ...
 *
 * @param string $entry Bundle basename (without extension).
 * @return array|false
 */
function threew_2025_get_asset($entry = 'index') {
    static $assets = [];

    if (!isset($assets[$entry])) {
        $asset_path = get_theme_file_path("build/{$entry}.asset.php");
        if (file_exists($asset_path)) {
            $assets[$entry] = include $asset_path;
        } else {
            $assets[$entry] = false;
        }
    }
...
add_action('enqueue_block_editor_assets', function () {
    $asset = threew_2025_get_asset('editor');
    $script_path = get_theme_file_path('build/editor.js');
...
function threew_handle_contact_form() {
	...
	// Prepare email
	$custom_recipient = sanitize_email(get_option('threew_about_contact_email', ''));
	$to = $custom_recipient ?: get_option('admin_email');
	$headers = [
